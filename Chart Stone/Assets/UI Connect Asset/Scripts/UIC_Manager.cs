...
public class UIC_Manager : MonoBehaviour
{
    ...
    // v2.2 - UIC_ContextMenu made not static to enable multiple charts
    public UIC_ContextMenu contextMenu;
    ...
    void Awake()
    {
        ...
        contextMenu = GetComponentInChildren<UIC_ContextMenu>();
    }
    ...
    public void InstantiateEntityAtPosition(UIC_Entity entityToInstantiate, Vector3 position)
    {
        ...
        // v2.2 - duplicated block is now instantiated with the same parent of the reference block 
        if (entityToInstantiate.transform.parent)
        {
            go.transform.SetParent(entityToInstantiate.transform.parent);
